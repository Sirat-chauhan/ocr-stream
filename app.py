...
# ── NEW: Upload face photo to Supabase Storage ────────────────────
def upload_photo_to_storage(photo_b64: str, doc_type: str) -> str:
    """Upload base64 face photo to Supabase Storage bucket, return signed URL."""
    ...
    try:
        ...
        url_res = supabase.storage.from_("id-photos").create_signed_url(
            file_path, 315360000
        )
        return url_res.get("signedURL", "") if isinstance(url_res, dict) else ""
    except Exception as e:
        log_failure("Photo Upload", str(e))
        return ""
...
def save_extraction(doc_type, fields, raw_text="", file_name="", file_size_bytes=0, photo_b64=None):
    ...
    if photo_b64:
        with st.spinner("📤 Uploading photo to storage..."):
            photo_url = upload_photo_to_storage(photo_b64, doc_type)

    # ── Attempt insert with all columns (including extended + photo_url) ──
    try:
        row = _build_row(fields, doc_type, file_name, size_kb, raw_text,
                         include_extended=True, photo_url=photo_url)
    ...
    try:
        row = _build_row(fields, doc_type, file_name, size_kb, raw_text,
                         include_extended=False, photo_url=photo_url)
        supabase.table("extractions").insert(row).execute()
        return True, "partial"
...
st.markdown(
    f"""<div style="display:flex;align-items:center;
        ...
        {"<span style='color:#9ca3af;font-size:0.76rem;margin-left:10px;'>· Last login: " + login_info + "</span>" if login_info else ""}
    </div>""",
    unsafe_allow_html=True)
                ...
                if "error" in result:
                    ...
                    if mode == "Document" and fields:
                        saved, save_err = save_extraction(
                            ...
                            photo_b64=photo_b64)   # ← passes photo for Storage upload
    ...
    if not records:
        ...
        for r in records:
            ...
            with st.expander(f"{dlabel} · {name}", expanded=False):
                ...
                # ── Show stored photo from Supabase Storage ──
